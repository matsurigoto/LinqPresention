...
namespace RestrictionOperators
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var productQuery = products.Select(p => new
            {
                p.ProductName,
                p.UnitPrice
            });
            ...
            {
                Console.WriteLine("Uppercase: " + item.ProductName + ", Lowercase: " + item.UnitPrice);
            }


            //--------------------------------SELECT------------------------------------//

            var upperLowerWords = products.Select(p => new
            {
                Upper = p.ProductName.ToUpper(),
                Lower = p.ProductName.ToLower()
            });
            ...
            //--------------------------------SELECT Many------------------------------------//
            var selectManyQuery = products.SelectMany(p => p.ProductName);
            foreach (var item in selectManyQuery)
            {
                Console.WriteLine(item);
            }

            //--------------------------------Find most unit in stock products------------------------------------//
            var query = products.OrderByDescending(p => p.UnitsInStock).ThenBy(x => x.ProductName).Take(10);

            foreach (var item in query)
            {
                Console.WriteLine("ProductName: " + item.ProductName + ", UnitsInStock: " + item.UnitsInStock);
            }
        }
