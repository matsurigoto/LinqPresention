...
namespace GroupOperators
{
    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //            group product by product.Category into categroyGroup
            //            orderby categroyGroup.Key
            //            select categroyGroup;

            var query = products.GroupBy(p => p.Category).OrderBy(g => g.Key);
            ...
            var query2 = products.GroupBy(p => p.Category)
                                .Select(g => new
                                {
                                    g.Key,
            ...
            foreach (var group in query2)
            {
                ...
                Console.WriteLine($"\t Min: {group.Average}");
            }


            //-----------------------------------Aggregating-------------------------------------//
            var anyResult = products.Any(p => p.Category == "Beverages");
            Console.WriteLine(anyResult);

            var allResult = products.All(p => p.Category == "Beverages");
            Console.WriteLine(allResult);

            var containResult = products.Where(p => p.Category.Contains("Beverages") );
